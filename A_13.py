# 80个特征版，7种特征组合，信心2,RF的“驾驶+眼动+EEG”混淆矩阵
...
for path in participant_paths:
    if os.path.exists(path):
        df = pd.read_csv(path)
        all_confidences.extend(df['Confidence_2'].values)
...
FS = 500  # EEG 采样率
FEATURE_SIZES = {
    'driving': 35 + 7,  # 窗口特征（35） + 新特征（7），去除了Collision
...
for path in participant_paths:
    ...
    for index, row in experiment_log.iterrows():
        ...
        if not steering_data.empty and not vehicle_data.empty and not eeg_data.empty and not pupil_data.empty:
            ...
            driving_features = np.concatenate([windowed_data.flatten(), driving_raw_features])  # 去除了collision
...
if any(results['RandomForest'][combo]['accuracies'] for combo in feature_combinations):
    ...
    for combo in feature_combinations:
        ...
        if combo == '驾驶+眼动+EEG':
            ...
            # plt.title(f'平均混淆矩阵 (准确率) - {combo}')
            plt.xlabel('Predicted Label')
            plt.ylabel('True Label')
            # 手动添加颜色条，使用热图的第一个 QuadMesh 对象
