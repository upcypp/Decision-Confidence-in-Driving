...
for path in participant_paths:
    try:
        ...
        # 确保所需列存在
        required_columns = ['Confidence_2', 'Initial_Distance', 'TTA', 'Collision']
        ...
        y = df_plot['Confidence_2'].values

        # 使用线性回归拟合 Confidence_2
        ...
        # 计算 MCE：结合信心倾向性和 R²
        mce = 100 * (0.45 * confidence_tendency + 0.55 * r2)  # 倾向性权重 0.4，R² 权重 0.6
        mce_scores[participant_id] = mce

        # 创建散点图
        plt.figure(figsize=(8, 6))
        coord_counts = Counter(zip(df_plot['Confidence_2'], df_plot['TTA']))
        x_coords = df_plot['Confidence_2']
        ...
        plt.xlabel('Confidence_2')
        plt.ylabel('TTA')
        plt.title(f'Scatter Plot of Confidence_2 vs TTA\nParticipant ID: {participant_id}\nMean MCE: {mce:.2f}/100')
