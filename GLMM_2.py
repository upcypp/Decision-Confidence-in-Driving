...
# 确保保留相关列
combined_data = combined_data[['Participant_ID', 'Experiment_Number', 'Initial_Distance', 'TTA', 'Confidence']]
...
# 将 Confidence 转换为分类变量并编码
combined_data['Confidence'] = pd.Categorical(combined_data['Confidence'], categories=[1, 2, 3, 4, 5], ordered=True)
combined_data['Confidence'] = combined_data['Confidence'].cat.codes + 1

# 定义模型公式
formula = "Confidence ~ Initial_Distance + TTA"
...
for i, initial_distance in enumerate(sorted(combined_data['Initial_Distance'].unique(), key=float)):
    ...
    # 按 TTA 分组计算个体趋势
    individual_trends = subset.groupby(['Participant_ID', 'TTA'])['Confidence'].mean().reset_index()

    # 按 TTA 分组计算总体平均值及标准误差
    average_trend = subset.groupby('TTA')['Confidence'].agg(['mean', 'sem']).reset_index()
    # 计算 95% 置信区间的误差范围
    ...
    for participant_id in individual_trends['Participant_ID'].unique():
        ...
        axes[i].plot(participant_data['TTA'], participant_data['Confidence'], linestyle='--', color='black', alpha=0.2)  # 移除 label
        axes[i].scatter(participant_data['TTA'], participant_data['Confidence'], color='black', alpha=0.2, s=10)
...
plt.savefig(os.path.join(save_dir, "linear_1.svg"), format="svg", bbox_inches='tight')
